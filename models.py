...
class Notification(Base):
    ...
    tweet_id: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
...
class Conversation(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    participant_a: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    participant_b: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    last_message_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())

    __table_args__ = (UniqueConstraint("participant_a", "participant_b", name="uq_conversation"),)
